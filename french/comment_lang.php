<?php

$lang = array(

//----------------------------------------
// Required for MODULES page
//----------------------------------------

"comment_module_name" =>
"Commentaires",

"comment_module_description" =>
"Module de commentaires",

//----------------------------------------


'comment_moved' =>
'Commentaire tranféré',

'comments_moved' =>
'Commentaires transférés',

'filter_comments' =>
'Filtrer les commentaires',

'comment' =>
'Commentaire',

'comments' =>
'Commentaires',

'edit_comment' =>
'Éditer les commentaires',

'settings' =>
'Paramètres',

'comment_settings' =>
'Paramètres de commentaire',

'delete_confirm' =>
'Confirmation de suppression de commentaire',

'delete_comment_confirm' =>
'Êtes-vous sûr de vouloir supprimer ce commentaire définitivement ?',

'delete_comments_confirm' =>
'Êtes-vous sûr de vouloir supprimer ces commentaires définitivement ?',

'comment_deleted' =>
'Commentaire supprimé',

'status_changed' =>
'Statut modifié',

'comment_word_censoring' =>
'Forcer la censure des mots pour les commentaires',

'comment_moderation_override' =>
'Modérer les articles ayant expiré',

'comment_smart_notifications' =>
'Activer les notifications intelligentes',

'selection_required' =>
'Avant de soumettre, faîtes au moins une sélection',

'close_selected' =>
'Positionner le statut à Fermé',

'open_selected' =>
'Positionner le statut à Ouvert',

'pending_selected' =>
'Positionner le statut à En Attente',

'move_selected' =>
'Déplacer la sélection',

'instructions' =>
'Instructions : ',

'move' =>
'Déplacer',

'move_comment_instr' =>
'Saisissez le nouveau numéro d\'ID article :',

'registered_member' =>
'Membre enregistré',

'url' =>
'URL',

'location' =>
'Ville, Pays',

'choose_entry_for_comment_move' =>
'Choisissez l\'article pour le déplacement de commentaire',

'move_comments_to_entry' =>
'Déplacer les commentaires vers l\'article',

'filter_by_channel' =>
'Filtrer par canal',


'filter_by_status' =>
'Filtrer par statut',

'filter_by_author' =>
'Filtrer par auteur',

'collapsed_display' =>
'Affichage regroupé',

'expanded_display' =>
'Affichage étendu',

'date_range' =>
'Plage de dates',

'keywords' =>
'Mots clefs',

'open' =>
'Ouvert',

'closed' =>
'Fermé',

'entry_title' =>
'Titre de l\'article',

'email' =>
'Email',

'name' =>
'Nom',

'ip_address' =>
'Adresse IP',

'delete_selected' =>
'Supprimer la sélection',


'blacklist' =>
'Ajouter les adresses IP à la liste noire',

'search_in_comments' =>
'Rechercher dans les commentaires',

'search_in_ips' =>
'Rechercher dans les adresses IP',

'search_in_entry_titles' =>
'Rechercher dans les titres d\'article',

'search_in_names' =>
'Rechercher dans les noms',

'search_in_emails' =>
'Rechercher dans les adresses email',

'today' =>
'Aujourd\'hui',

'past_day' =>
'Dernières 24 heures',

'past_week' =>
'7 derniers jours',

'past_month' =>
'30 derniers jours',

'past_six_months' =>
'180 derniers jours',

'past_year' =>
'365 derniers jours',

'any_date' =>
'Plage de dates personnalisée',


//

"cmt_commenting_has_expired" =>
"Les commentaires ne sont pas disponibles pour cet article.",

"cmt_name_not_allowed" =>
"Le nom avec lequel vous souhaiter publier est réservé. Choisissez un nom différent.",

"cmt_must_be_member" =>
"Vous devez être un membre inscrit pour publier des commentaires",

"cmt_no_authorized_for_comments" =>
"Vous n'êtes pas autorisé à publier des commentaires",

"cmt_account_not_active" =>
"Votre compte de membre n'a pas encore été activé",

"cmt_comments_not_allowed" =>
"Les commentaires ne sont pas autorisés pour ce canal",

"cmt_missing_name" =>
"Le champ nom est requis",

"cmt_missing_comment" =>
"Le champ commentaire est requis",

"cmt_missing_email" =>
"Le champ email est requis",

"cmt_invalid_email" =>
"L'adresse email que vous avez saisie est invalide",

"cmt_banned_email" =>
"L'adresse email que vous avez saisie est exclue",

"cmt_too_large" =>
"Le commentaire que vous avez saisi contient %n caractères. Seuls %x caractères sont autorisés.",

"cmt_comments_timelock" =>
"Vous n'êtes autorisé à publier des commentaires que toutes les %s secondes.",

"cmt_duplicate_comment_warning" =>
"Impossible de recevoir votre commentaire pour le moment.",

"cmt_no_preview_template_specified" =>
"La balise du formulaire de commentaire ne contient pas d'information sur l'emplacement de votre modèle d'aperçu.",

"cmt_comment_response_title" =>
"Quelqu'un a répondu à votre commentaire",

"cmt_comment_response_body" =>
"Quelqu'un a répondu à l'article auquel vous vous êtes abonné à :",

"cmt_comment_title" =>
"Le titre de l'article est :",

"cmt_comment_url" =>
"Vous pouvez consulter le commentaire à l'adresse suivante :",

"cmt_comment_removal" =>
"Pour ne plus recevoir de notifications pour ce commentaire, cliquez ici :",

"cmt_notification_removal" =>
"Arrêt des notifications",

"cmt_you_have_been_removed" =>
"Vous êtes désinscrit et ne recevrez plus les futures notifications.",

"cmt_comment_accepted" =>
"Commentaire accepté",

"cmt_will_be_reviewed" =>
"Votre commentaire sera soumis à un modérateur pour validation.",

"cmt_return_to_comments" =>
"Retourner aux commentaires",

'you_have_been_subscribed' =>
'Vous suivez dorénavant l\'article suivant :',

'you_have_been_unsubscribed' =>
'Vous ne suivez dorénavant plus l\'article suivant :',

'cmt_subscribe' =>
'Abonnez-vous à ce commentaire',

'cmt_unsubscribe' =>
'Désabonnez-vous à ce commentaire',

'invalid_entry_id' =>
'ID d\'article invalide',

'no_valid_comments' =>
'Aucun commentaire valide',

'comment_edit_time_limit' =>
'Limite du temps d\'édition des commentaires (en secondes, mettre 0 pour aucune limite)',

'already_subscribed' =>
'Vous êtes déjà abonné à ce commentaire.',

'not_currently_subscribed' =>
'Il ne semble pas que vous soyez abonné à l\'ID d\'article que vous avez soumise',

'invalid_subscription' =>
'L\'URL soumise est invalide',

'settings_updated' =>
'Paramètres mis à jour',

'comment_updated' =>
'Commentaire mis à jour',

'start_date' =>
'Date de début',

'end_date' =>
'Date de fin',

'expand' =>
'Développer',

'no_results' =>
'Aucun résultat',

''=>''
);

/* End of file comment_lang.php */
/* Location: ./system/expressionengine/language/french/comment_lang.php */